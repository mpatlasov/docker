...
func Init(home string, input_options []string) (graphdriver.Driver, error) {
	...
	if !foundGraphDriver {
		log.Infof("Init: graphdriver not specified, using devicemapper")
	...
	if err != nil {
		log.Errorf("ProxyAPI.Init: %s", err)
		return nil, err
	}
...
func (d *Driver) Status() [][2]string {
	...
	if err != nil {
		log.Errorf("ProxyAPI.Status: %s", err)
		return nil
	}
	return reply.Status
}
...
func (d *Driver) Cleanup() error {
	...
	if err != nil {
		log.Errorf("ProxyAPI.Cleanup: %s", err)
	}
	return err
}
...
func (d *Driver) Create(id, parent string) error {
	...
	if err != nil {
		log.Errorf("ProxyAPI.Create: %s", err)
	}
	return err
}
...
func (d *Driver) Remove(id string) error {
	...
	if err != nil {
		log.Errorf("ProxyAPI.Remove: %s", err)
	}
	return err
}
...
func (d *Driver) Exists(id string) bool {
	...
	if err != nil {
		log.Errorf("ProxyAPI.Exists: %s", err)
	}
	return reply.Exists
}
...
func (d *Driver) GetMetadata(id string) (map[string]string, error) {
	...
	if err != nil {
		log.Errorf("ProxyAPI.GetMetadata: %s", err)
	}
